...
if ($cekemail > 0) {
    ...
    if ($cek > 0) {
        ...
        $token = baca_tabel('login', 'token', "where id_user = '$id_user' and device_data = '$device_data'");
        if (!$token) {
            ...
            $data['waktu'] = date("Y-m-d H:i:s");
            $update = update_tabel('login', $data, "where id_user = '$id_user' and device_data = '$device_data'");
            if ($update) {
                $dataRes['code'] = 200;
                $dataRes['msg'] = 'Berhasil Login';
                $dataRes['token'] = $token;
