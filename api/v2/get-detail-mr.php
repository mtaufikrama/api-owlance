...
include '../../_lib/function/uang.php';
...
while($get_pasien=$run_pasien->fetchRow()) {
    ...
    if($get_pasien['foto']) $get_pasien['foto'] = $url . $get_pasien['foto'];
    $get_pasien['tgl_lhr'] = substr($get_pasien['tgl_lhr'], 0, 10);
    if($get_pasien['provinsi'])     $get_pasien['provinsi'] = baca_tabel('dc_propinsi', 'nama_propinsi', "where id_dc_propinsi='" . $get_pasien['provinsi'] . "'");
    if($get_pasien['kota'])         $get_pasien['kota'] = baca_tabel('dc_kota', 'nama_kota', "where id_dc_kota='" . $get_pasien['kota'] . "'");
    if($get_pasien['kecamatan'])    $get_pasien['kecamatan'] = baca_tabel('dc_kecamatan', 'nama_kecamatan', "where id_dc_kecamatan='" . $get_pasien['kecamatan'] . "'");
    if($get_pasien['kelurahan']){
...
/* --- get Vital Sign --- */
$sql_vs="SELECT keadaan_umum,kesadaran_pasien,tekanan_darah,nadi,suhu,pernafasan,tinggi_badan,berat_badan from gd_th_rujuk_ri where no_registrasi='$no_registrasi'";
...
/* --- get SOAP --- */
$sql_soap="SELECT a.status_pasien as Subyektive,a.terapi as Analyst,b.keterangan as Objective from tc_status_pasien a left join tc_soap b on a.no_registrasi=b.no_registrasi where a.no_registrasi='$no_registrasi'";
...
/* --- get Tindakan --- */
$not = 1;
$SqlgetTindakan="SELECT * FROM tc_trans_pelayanan where no_mr='$no_mr' AND no_kunjungan=$no_kunjungan AND kode_bagian like '01%'";
...
/* --- get ICD 10 --- */
$noi = 1;
$Sql_icd10="SELECT kode_icd_pasien as id_icd_pasien, kode_icd, kelompok_icd, kode_asterik from th_icd10_pasien where no_registrasi=$no_registrasi";
...
/* --- get Resep --- */
$no = 1;
$sql_resep="SELECT a.tgl_pesan,b.* FROM fr_tc_pesan_resep_dr as a 
...
while($get_resep=$run_resep->fetchRow()) {
    ...
    $nama_dosis = $get_resep["nama_dosis"];
