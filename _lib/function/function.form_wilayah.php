...
<html>
	...
	<body scroll="no">
		...
		<!-- ========================================================================================= -->
		<div id="isiUtama">		
			<form method="post" name="PUNK" action="<?WWWROOT?>/_lib/function/function.form_wilayah.php">
		...
		<script language="JavaScript" type="text/javascript">
			...
			function cekDulu(tipeCari,Elm){
				if (Elm.value!=""){
					var url = HTTPPATH+"/_lib/function/function.form_wilayah.php?param=AJAX&tipe_cari="+tipeCari+"&filter=" + Elm.value;
...
?>
	...
	<!-- START javascript #######################################################################-->
		...
		if(HTTPPATH == null)var HTTPPATH=location.protocol + '//' + location.host;
		var AJAXPATH			= HTTPPATH+"/_lib/function/function.form_wilayah.php";
		...
		function getData_<?=$kode_pos?>(obj) {
			...
			function tambah_wilayah_<?=$kode_pos?>(Elm){
				...
				if (tipeCari!='' && idAtas!='' && namaCari!='') {
					url = HTTPPATH+"/_lib/function/function.form_wilayah.php?param=ADD_WILAYAH&tipe_cari="+tipeCari+"&id_atas="+idAtas+"&nama_cari="+namaCari+"&data_baru="+dataBaru_<?=$kode_pos?>+"&id_pisah=<?=$kode_pos?>";
					openPop(url);
				} else {
			...
			function refreshMe(tipeCari,selectedElm,idPisah) {
				...
				var url = HTTPPATH+"/_lib/function/function.form_wilayah.php?param=AJAX&tipe_cari=" + tipeCari + optUrl;
