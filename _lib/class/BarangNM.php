...
if (!class_exists("BarangNM")) {
	class BarangNM extends AvObjects {
		...
		public function _setKodeBarang($kodeBarang) {
			...
			$rBrg=read_tabel("mt_barang_nm","*","WHERE kode_brg='".$kodeBarang."'");
		...
		public function tambah_depo_stok($kodeBrg,$jumlah,$kodeBagian) {
			if (is_numeric($jumlah)) {
				$jml_sat_kcl=baca_tabel("mt_depo_stok_nm","jml_sat_kcl","WHERE kode_brg='".$kodeBrg."' AND kode_bagian='".$kodeBagian."'");
				$jml_sat_kcl_sekarang = $jml_sat_kcl + $jumlah;

				$fld=array();
				$fld["jml_sat_kcl"] = $jml_sat_kcl_sekarang;
				update_tabel("mt_depo_stok_nm",$fld,"WHERE kode_brg='".$kodeBrg."' AND kode_bagian='".$kodeBagian."'");
			}
		...
		public function tambah_kartu_stok($kodeBrg,$jumlah,$kodeBagian,$jenisKartuStok,$keterangan="") {
			if (is_numeric($jumlah)) {
				$jml_sat_kcl=baca_tabel("mt_depo_stok_nm","jml_sat_kcl","WHERE kode_brg='".$kodeBrg."' AND kode_bagian='".$kodeBagian."'");
				$jml_sat_kcl_sekarang = $jml_sat_kcl + $jumlah;
				$id_kartu=max_kode_number("tc_kartu_stok_nm","id_kartu");
				...
				insert_tabel("tc_kartu_stok_nm",$fld);
			}
