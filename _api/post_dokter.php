...
include "../api/v1/cek-token.php";
include "../api/encrypt.php";
...
if ($perintah == 'show' || $perintah == 'SHOW') {
...
} elseif ($perintah == 'insert' || $perintah == 'alter' || $perintah == 'update' || $perintah == 'delete' || $perintah == 'drop' || $perintah == 'INSERT' || $perintah == 'ALTER' || $perintah == 'UPDATE' || $perintah == 'DELETE' || $perintah == 'DROP' || $perintah == 'rename' || $perintah == 'RENAME') {
    $result = $conn->query($sql);
    if ($result === false) {
        $data['code']=500;
        $data['msg']="Error : " . $conn->error;
    ...
    $data['item']=["SELECT","st","sf","cdb","iiv","dt","cud","cs","ddb"];
}
